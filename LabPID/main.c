...
#define DISTANCE_SLOPE_RISE 279 //TODO: Calibrate to your own sensor! See Port6 IRQH to see how these are used.
#define DISTANCE_SLOPE_RUN 208
#define DISTANCE_OFFSET -148
...
void configTA2andGPIOsForUltrasoundDistSensor(){
    ...
    GPIO_setAsInputPin(GPIO_PORT_P6, GPIO_PIN1); //no pullup/pulldown resistor needed; signal output from distance sensor
    GPIO_interruptEdgeSelect(GPIO_PORT_P6, GPIO_PIN1, GPIO_LOW_TO_HIGH_TRANSITION); // watching for rising edge
...
void driveIndefinitely(bool forward, uint16_t PWM){ //LOOK_HERE
    if(forward){
            GPIO_setOutputHighOnPin(GPIO_PORT_P4, GPIO_PIN1 | GPIO_PIN3);
            GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN2 | GPIO_PIN4);
        } else {
            GPIO_setOutputLowOnPin(GPIO_PORT_P4, GPIO_PIN1 | GPIO_PIN3);
            GPIO_setOutputHighOnPin(GPIO_PORT_P4, GPIO_PIN2 | GPIO_PIN4);
        }
...
void PORT6_IRQHandler(){
    ...
    if(status & GPIO_PIN1){
        if(distance_riseTimestamp == 0){ // we haven't seen the rising edge yet this cycle
            ...
            GPIO_interruptEdgeSelect(GPIO_PORT_P6, GPIO_PIN1, GPIO_HIGH_TO_LOW_TRANSITION); // watch for falling edge
            ...
            distance_riseTimestamp = 0;
            GPIO_interruptEdgeSelect(GPIO_PORT_P6, GPIO_PIN1, GPIO_LOW_TO_HIGH_TRANSITION);
